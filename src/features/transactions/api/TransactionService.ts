...
export const TransactionService = {
	...
	getSpendingHeatmap: async (
		req: SpendingHeatmapRequest,
	): Promise<SpendingHeatmapResponse[]> => {
		const response = await axiosInstance.post<SpendingHeatmapResponse[]>(
			`${TRANSACTION_PREFIX}/spending-heatmap`,
			req,
		);
	...
	getFixedVsVariable: async (
		req: FixedVsVariableRequest,
	): Promise<FixedVsVariableResponse> => {
		const response = await axiosInstance.post<FixedVsVariableResponse>(
			`${TRANSACTION_PREFIX}/fixed-vs-variable`,
			req,
		);
	...
	getRecurringTransactions: async (
		req: RecurringTransactionsRequest,
	): Promise<RecurringTransactionResponse[]> => {
		const response = await axiosInstance.post<RecurringTransactionResponse[]>(
			`${TRANSACTION_PREFIX}/recurring`,
			req,
		);
